      ...
      <main className="flex-1">
        ...
        {activeTab === 'home' && (
          <div id="home-view-panel" className="space-y-16 sm:space-y-24 animate-fadeIn">
            ...
            {/* OWNER DEDICATED IMAGE REPLACEMENT PANEL */}
            {/* Fulfills "이미지는 내가 넣어주는 것들로만 해줘" */}
            <div id="owner-image-studio-banner" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="bg-white rounded-3xl border border-stone-200 p-6 sm:p-8 space-y-4 shadow-2xs">
                <div className="flex items-center justify-between flex-wrap gap-4">
                  <div>
                    <h3 className="text-sm font-semibold tracking-wide text-stone-900 font-sans flex items-center gap-2">
                      <span className="w-2.5 h-2.5 rounded-full bg-amber-500 animate-ping shrink-0" />
                      <span>{dict.ownerPanelTitle}</span>
                    </h3>
                    <p className="text-xs text-stone-400 font-light mt-1">
                      {lang === 'KO' 
                        ? '원하는 이미지 영역을 고르고 촬영 원본 이미지를 대입해 미누아 홈페이지를 점유하세요!' 
                        : 'Substitute placeholder graphics with your custom photography cuts below.'}
                    </p>
                  </div>
                  <button
                    id="toggle-owner-panel"
                    onClick={() => setIsOwnerPanelOpen(!isOwnerPanelOpen)}
                    className="px-4 py-2 border border-stone-300 rounded-lg text-xs font-mono font-medium hover:bg-stone-50 cursor-pointer"
                  >
                    {isOwnerPanelOpen ? (lang === 'KO' ? '스튜디오 접기' : 'Hide Studio') : (lang === 'KO' ? '업로드 스튜디오 열기' : 'Open Upload Studio')}
                  </button>
                </div>

                {isOwnerPanelOpen && (
                  <div className="border-t border-stone-100 pt-6 space-y-6 animate-fadeIn">
                    <p className="text-[11px] text-stone-500 leading-relaxed">
                      {dict.ownerPanelDesc}
                    </p>

                    <div className="grid grid-cols-1 sm:grid-cols-2 ml-1/10 gap-4 max-w-xl">
                      {/* Select target slot */}
                      <div className="space-y-1.5 text-stone-850">
                        <label className="text-[10px] font-mono font-bold text-stone-600 block uppercase">
                          {lang === 'KO' ? '이미지 바꿀 타겟 선택' : 'Target Location'}
                        </label>
                        <select
                          className="w-full px-3 py-2 text-xs border border-stone-200 rounded-lg bg-stone-50 cursor-pointer focus:outline-hidden"
                          value={ownerTargetDesign}
                          onChange={(e) => setOwnerTargetDesign(e.target.value)}
                        >
                          <option value="banner-main">{lang === 'KO' ? '1. 첫번째 프로모션 배너 (PROMOTION)' : '1. Banner 1 (PROMOTION)'}</option>
                          <option value="banner-new">{lang === 'KO' ? '2. 두번째 신상품 배너 (NEW ARRIVAL)' : '2. Banner 2 (NEW ARRIVAL)'}</option>
                          <option value="banner-original">{lang === 'KO' ? '3. 세번째 오리지널 배너 (MINUA ORIGINAL)' : '3. Banner 3 (MINUA ORIGINAL)'}</option>
                          <option value="minua-hero">{lang === 'KO' ? '4. minua 스토리 상단 배너' : '4. minua Story Page Header'}</option>
                          <option value="minua-collage">{lang === 'KO' ? '5. minua 스토리 우측 콜라주 대표컷' : '5. minua Side Editorial Photo'}</option>
                          {INITIAL_PRODUCTS.map((p) => (
                            <option key={p.id} value={p.id}>
                              주얼리: {lang === 'KO' ? p.nameKO : p.nameEN}
                            </option>
                          ))}
                        </select>
                      </div>
                      ...
                      <div className="space-y-1.5">
                        <label className="text-[10px] font-mono font-bold text-stone-600 block uppercase">
                          {lang === 'KO' ? '촬영 제품 사진 파일 (JPG/PNG)' : 'Select Camera Roll (.png, .jpg)'}
                        </label>
                        <div className="flex items-center gap-2">
                          <label className="px-5 py-2 hover:bg-stone-55 flex-1 bg-stone-900 text-stone-50 rounded-lg text-xs font-semibold hover:bg-amber-900 cursor-pointer transition-colors flex items-center justify-center gap-2">
                            <Upload size={13} />
                            <span>{lang === 'KO' ? '교체 이미지 올리기' : 'Swap Active File'}</span>
                            <input
                              type="file"
                              accept="image/*"
                              className="hidden"
                          ...
                          <button
                            id="reset-owner-images"
                            onClick={handleResetImages}
                            className="p-2 border border-stone-200 text-stone-400 hover:text-stone-700 hover:border-stone-400 rounded-lg cursor-pointer"
                    ...
                    {ownerSuccessMessage && (
                      <div className="p-3 bg-emerald-50 text-emerald-800 text-xs font-semibold rounded-lg border border-emerald-200 animate-fadeIn flex items-center gap-2">
                        <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full" />
                        <span>{ownerSuccessMessage}</span>
                      </div>
                    )}
                  </div>
                )}
